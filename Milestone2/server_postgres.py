...
load into PostgresSQL database using python.
"""
...
class TweetPrinter(tweepy.Stream):
	...
	def split_time_phrase(self, line, timegroup):
		"""
		We split the time stamp and word.
		Also, split the tweets into each phrase and prepare 
		the data so we can insert into database using psycopg
		Input:
			line (list): include timestamp and tweets
			time_group (datetime): timestamp without second of each tweet - in that certain minutes
		"""
		time = line[0]
		words_ls = line[1]
	...
	def split_time_word(self, line, timegroup):
		"""
		We split the time stamp and word.
		Also, split the tweets into each word and prepare 
		the data so we can insert into database using psycopg
		Input:
			line (list): include timestamp and tweets
			time_group (datetime): timestamp without second of each tweet - in that certain minutes
	...
	def insert_to_table(self, time, time_group, phrase, count):
		"""
		Insert needed data into database using psycopg
		Input:
			time (datetime): timestamp of each tweet
			time_group (datetime): timestamp without second of each tweet - in that certain minutes
			phrase (text): input phrase in the tweets
